...
public class FonctionnalitesDao implements StbDAO {
	...
	public void saveOrUpdate(STB stb) {
		...
		String sql = "INSERT INTO fonctionnalites (description, priorite, id_stb) VALUES ('" + descriptionFonctionnalite
				+ "','" + prioriteFonctionnalite + "','" + stbId + "')";
	...
	public StbFonctionnalites get(int stbId) {
		...
		return jdbcTemplate.query(sql, new ResultSetExtractor<StbFonctionnalites>() {
			@Override
			public StbFonctionnalites extractData(ResultSet rs) throws SQLException, DataAccessException {
				if (rs.next()) {
